...
export const sos = reactive({
  ...
  showContactModal: (event) => {
    ...
    sos.contactMode = event.mode ? event.mode : 'contact_us'
    if (window.innerWidth >= 640 || window.innerHeight >= 800) {
      ...
      router.push({
        name: sos.contactMode,
        query: {
          event_source: sos.eventSource,
          text: sos.contactText,
          mode: 'contact'
        }
      })
    }
  ...
  showCourseDetails: (event) => {
    ...
    sos.registrationText = event.registrationText
    if (window.innerWidth >= 650 || window.innerHeight >= 850) {
